...
def analyze_spending(spending_data, monthly_budget):
    ...
    if total_spent > monthly_budget:
        ...
        tips.append(f"🎯 이번 달 예산의 {monthly_budget - total_spent:,}원이 남았습니다. 남은 금액은 비상금 계좌나 예적금으로 저축해보는 건 어떨까요?")
    ...
    for item in spending_data:
        ...
        if item["category"] == "카페" and item["amount"] > 70000:
            ...
            tips.append("🛍️ 쇼핑 지출이 높습니다. ‘소비하지 않는 날(No-Spend Day)’을 정해보세요.")
        elif item["category"] == "식비" and item["amount"] > 200000:
            tips.append("🍱 식비가 많다면 일주일에 하루는 도시락을 싸거나 집밥 위주로 구성해보세요.")
    ...
    random.shuffle(tips)
    return tips[:max(2, len(tips))]  # 최소 2개 이상 조언 표시
...
fig, ax = plt.subplots(figsize=(10, 5))
avg_df.plot(kind="bar", legend=False, ax=ax, color="#e67e22")
ax.set_ylabel("지출 금액 (원)", fontproperties=fontprop)
ax.set_xlabel("카테고리", fontproperties=fontprop)
